...
define(['jquery', 'oae.core', 'jquery.history'], function($, oae) {
	...
	console.log(widgetData);
		...
		console.log(History.getState());
            ...
            if ((widgetData.canAdd || widgetData.canManage)) {
                initialContent = oae.api.util.template().render($('#bbb-list-actions-template', $rootel));
		}


            var url = '/api/bbb/' + widgetData.context.id;
		console.log(url);
            if (query) {
		console.log(query);
                url = '/api/search/bbb/' + widgetData.context.id;
            }
		...
		'limit': 12,
			...
			'#bbb-template',
			{
                ...
                'emptyListProcessor': function() {
                    oae.api.util.template().render($('#bbb-noresults-template', $rootel), {
                        'query': query
                    }, $('.oae-list', $rootel));
                }
            });
        ...
        var setUpListOptions = function() {
                oae.api.util.template().render($('#bbb-list-options-actions-template', $rootel), {
         ...
         */
        var search = function() {
            var query = $('#bbb-search-query', $(this)).val();
        ...
        var addBinding = function() {
            // Set up search event
            $rootel.on('submit', '#bbb-search-form', search);
            ...
            $(window).on('oae.deleteresources.done', function() {
                ...
                if ($rootel.is(':visible')) {
                    ...
                    $('#bbb-selectall', $rootel).prop('checked', false);
                }
            });
        };
        addBinding();
